...
def wave_function_cycle(psi_j, x_grid, dx):
    ...
    while converged == False:
        ...
        cycles += 1
        if (np.abs(E_low - E_high) < shooting_method_epsilon):
            converged = True
            break
...
def main():
    ...
    while self_consistent <= 3:
        ...
        cycle += 1
        print(f"Waveform cycle number: {cycle}, energy found: {E_found}, energy found again: {E_again}, energy difference: {np.abs(E_found - E_again)}")
